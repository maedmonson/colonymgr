...
class YardColoniesTests(TestCase):
    def setUp(self):
        User.objects.create_user(username='john', email='john@doe.com', password='123')
        Yard.objects.create(description="This is a test", created_by = User.objects.first())

    def test_yard_colonies_view_success_status_code(self):
        url = reverse('yard_colonies', kwargs={'pk': 1})
        response = self.client.get(url)
        self.assertEquals(response.status_code, 200)

    def test_yard_colonies_view_not_found_status_code(self):
        url = reverse('yard_colonies', kwargs={'pk':99})
        response = self.client.get(url)
        self.assertEquals(response.status_code,404)

    def test_yard_colonies_url_resolves_yard_view(self):
        view = resolve('/yards/1/')
        self.assertEquals(view.func, yard_colonies)
